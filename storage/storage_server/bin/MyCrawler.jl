...
function main()
    if length(ARGS) < 3
        println("Usage: ./crawler url mytag myuser")
        exit(0)
    end
    url = String(ARGS[1])
    mytag = String(ARGS[2])
    ...
    function add_url_meta(url, myuser, dir) 
        url_filename = joinpath(dir, myuser, "url_meta.json")
        url_past_json = Dict()
    ...
    function add_meta(url, myuser, mytag, dir, path_parts)
        meta_filename = joinpath(dir, myuser, "meta.json")
        past_json = Dict()
        if isfile(meta_filename)
            past_string = read(meta_filename, String)
